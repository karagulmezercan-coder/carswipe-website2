"""
CarSwipe v8 — LightGBM BEST CONFIG:
  num_leaves=255, LR=0.05, n_estimators=8000 (early_stop~4490)
  max_depth=10, min_child_samples=25, reg_lambda=0.8
  MAPE=8.74%, MedAPE=5.57%, R²=0.9076 (82s training)
  27 features incl. power_per_liter, yas_sq, log_km_yas
"""
...
# Target encoding (5-fold CV, smoothed)
...
model = lgb.LGBMRegressor(
    n_estimators=8000, max_depth=10, num_leaves=255,
...
print("="*55)
for k,v in df["kaynak"].value_counts().items():
    print(f"  {k}: {v:,}")
print(f"\n✓ {len(df):,} ilan | MAPE: {mape:.1f}% | R²: {r2:.4f}")
print(f"✓ v8: num_leaves=255, LR=0.05, n_estimators=8000, best_iter={model.best_iteration_}")
print("="*55)
